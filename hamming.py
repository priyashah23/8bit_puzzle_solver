    ...
    def solution(self):
        ...
        while not (pq.empty()):
            ...
            for neighbour in neighbours:
                cost = no_of_moves + neighbour.get_hamming()
                ...
                if not (self.find_existing_board(pq, neighbour)):
                    pq.put(PrioritizedItem(cost, Hamming_Solver(neighbour, current_node, no_of_moves)))
                else:
...
class board:
    ...
    def hamming(self) -> int:
        ...
        for i in range(len(self.matrix)):
            if self.matrix[i] != 0 and self.matrix[i] != goal_state[i]:
    ...
    def neighbours(self) -> list:
        ...
        for n in neighbour:
            ...
            new_array[n] = tmp
            #this_board = board(new_array)
            new_boards.append(board(new_array))

        return new_boards

    def is_goal(self) -> bool:
        goal_board = board(matrix=np.array([1, 2, 3, 4, 5, 6, 7, 8, 0]))
